...
import { UserRoleDetail } from '../../models/user-role-detail.model';
...
export class UserInfoComponent implements OnInit {
    ...
    private user: UserDisplay = new UserDisplay();
    private userEdit: UserEditDisplay;
    private allRoles: UserRoleDetail[] = [];
  ...
  public compareRoles(r1: UserRoleDetail, r2: UserRoleDetail): boolean {
      ...
      this.alertService.startDefaultLoadingMessage();
        ...
        if (this.canViewAllRoles) {
          ...
          this.accountService.getUser().subscribe(
            user => this.onCurrentUserDataLoadSuccessful(user, user.roles.map(x => new UserRoleDetail(x.id, x.name))),
            error => this.onCurrentUserDataLoadFailed(error));
        }
    }

    private onCurrentUserDataLoadSuccessful(user: User, roles: UserRoleDetail[]) {
    ...
    private edit() {
        if (!this.isGeneralEditor) {
            ...
            this.userEdit = new UserEditDisplay();
    ...
    private cancel() {
        if (this.isGeneralEditor)
            this.userEdit = this.user = new UserEditDisplay();
        else
            this.userEdit = new UserEditDisplay();
    ...
    private unlockUser() {
        this.isSaving = true;
        this.alertService.startDelayedMessage("Unblocking user...");
    ...
    newUser(allRoles: Role[]) {
        ...
        this.allRoles = allRoles.map( r => new UserRoleDetail(r.id, r.name) );
        this.editingUserName = null;
        this.user = this.userEdit = new UserEditDisplay();
    ...
    displayUser(user: UserDisplay, allRoles?: Role[]) {
    ...
    private setRoles(user: User, allRoles?: Role[]) {
      ...
      this.allRoles = allRoles ? allRoles.map(r => new UserRoleDetail(r.id, r.name)) : [];

      if (user.roles) {
          // use has roles
          for (let ur of user.roles) {
            if (!this.allRoles.some(r => r.id == ur.id)) {
              // the role ur is not in 'allRoles' (maybe user can't read all roles from the server?)
              // add the role to the list of all roles
              this.allRoles.unshift(new UserRoleDetail(ur.id, ur.name));
            }
          }
        }
