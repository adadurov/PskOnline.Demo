...
export class RolesManagementComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.columns = [
          ...
          { prop: 'name', name: gT('roles.mgmt.Name'), width: 200 },
          { prop: 'description', name: gT('roles.mgmt.Description'), width: 350 },
          { prop: 'usersCount', name: gT('roles.mgmt.Users'), width: 80 },
    ...
    deleteRole(row: Role) {
      this.alertService.showDeleteWarning("roles.mgmt.roleAccusativeCase", row.name, () => this.deleteRoleHelper(row));
    }
