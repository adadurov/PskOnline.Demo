...
export class AlertService {
  ...
  showDeleteWarning(entityInAccusativeCase: string, entityName: string, okCallback: (val?: any) => any) {
    var entityTypeAccusative = this.translationService.getTranslation(entityInAccusativeCase);
    var message = this.translationService.getTranslation("alerts.deletionWarning", { entity: entityTypeAccusative, name: entityName });
    this.showDialog(message, DialogType.confirm, okCallback);
  }

  showOperationCancelledMessage() {
    this.showMessage(
      this.translationService.getTranslation("alerts.cancelled"),
      this.translationService.getTranslation("alerts.cancelledByUserNoChangesMade"),
    ...
    startDefaultLoadingMessage() {
      var message = this.translationService.getTranslation("alerts.loading")
      this.startDelayedMessage(message, "");
    }

    startDefaultSavingMessage() {
      var message = this.translationService.getTranslation("alerts.saving")
      this.startDelayedMessage(message, "");
    }

    startDelayedMessage(message: string, caption: string = "") : void {
      if (message === undefined) {
        message = this.translationService.getTranslation("alerts.pleaseWait")
      }
